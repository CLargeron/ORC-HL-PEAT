  ...
  SUBROUTINE aggregate_2d (nbpt, lalo, neighbours, resolution, contfrac, &
   ...
   &                   '(',domain_maxlat,',',domain_maxlon,')'
    ...
    ELSE
       ...
       write(*,*) 'interpol 250: domain_maxlon,domain_maxlat=',domain_maxlon,domain_maxlat
       write(*,*) 'domain_minlon,domain_minlat=',domain_minlon,domain_minlat
       write(*,*) 'loup_rel=',loup_rel(1,1)
       write(*,*) 'laup_rel=',laup_rel(1,1)
       write(*,*) 'lolow_rel=',lolow_rel(1,1)
       write(*,*) 'lalow_rel=',lalow_rel(1,1)
          ...
          DO jp=1,jml
             IF ( loup_rel(ip,jp) >= domain_minlon .AND. lolow_rel(ip,jp) <= domain_maxlon .AND.&
               &  laup_rel(ip,jp) >= domain_minlat .AND. lalow_rel(ip,jp) <= domain_maxlat ) THEN
                IF (mask(ip,jp) == 1 ) THEN
                   nbind = nbind + 1
!                   write(*,*) 'interpol 251: ip,jp,nbind=',nbind
                   searchind(nbind,1) = ip
                   searchind(nbind,2) = jp
                ENDIF
             ENDIF
