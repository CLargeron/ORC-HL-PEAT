...
MODULE routing
  ...
  !
!! PARAMETERS
  INTEGER(i_std), PARAMETER                                  :: nbasmax=7                   !! The maximum number of basins we wish to have per grid box (truncation of the model) (unitless)
  INTEGER(i_std)                                             :: nbvmax                      !! The maximum number of basins we can handle at any time during the generation of the maps (unitless)
  !
  ...
  !
  REAL(r_std), PARAMETER                                     :: beta = 2.0                  !! Parameter to fix the shape of the floodplain (>1 for convex edges, <1 for concave edges) (unitless)
  REAL(r_std), PARAMETER                                     :: betap = 0.5                 !! Ratio of the basin surface intercepted by ponds and the maximum surface of ponds (unitless;0-1)
  REAL(r_std), PARAMETER                                     :: floodcri = 2000.0           !! Potential height for which all the basin is flooded (mm)
  !
  ...
  REAL(r_std), PARAMETER                                     :: pondcri = 2000.0            !! Potential height for which all the basin is a pond (mm)
  !
  REAL(r_std), PARAMETER                                     :: maxevap_lake = 7.5/86400.   !! Maximum evaporation rate from lakes (kg/m^2/s)
  !
  REAL(r_std), PARAMETER                                     :: min_reservoir = 1.E-6_r_std !! A minimum amount of water in the reservoir is set
                                                                                            !! so that we do not loose when correcting for rounding errors (kg)
  !
  REAL(r_std), PARAMETER                                     :: crop_coef = 1.0             !! Empirical crop coefficient dependent on vegetation characteristics
...
!_ ================================================================================================================================
       ...
       IF ( doirrigation .OR. dofloodplains .OR. doswamps) THEN
          ...
          IF ( doirrigation ) THEN 
             ...
             IF ( .NOT. almaoutput ) THEN
                CALL histwrite(hist_id, 'irrigmap', 1, irrigated, nbpt, index)
             ELSE
                CALL histwrite(hist_id, 'IrrigationMap', 1, irrigated, nbpt, index)
             ENDIF
             IF ( hist2_id > 0 ) THEN
                IF ( .NOT. almaoutput ) THEN
                   CALL histwrite(hist2_id, 'irrigmap', 1, irrigated, nbpt, index)
                ELSE
          ...
          IF ( dofloodplains ) THEN
             ...
             IF ( .NOT. almaoutput ) THEN
                CALL histwrite(hist_id, 'floodmap', 1, floodplains, nbpt, index)
             ELSE
                CALL histwrite(hist_id, 'FloodplainsMap', 1, floodplains, nbpt, index)
             ENDIF
             IF ( hist2_id > 0 ) THEN
                IF ( .NOT. almaoutput ) THEN
                   CALL histwrite(hist2_id, 'floodmap', 1, floodplains, nbpt, index)
                ELSE
    ...
    IF ( NINT(time_counter) .GE. NINT(dt_routing) ) THEN 
       ...
       CALL routing_flow(nbpt, dt_routing, floodout_mean, runoff_mean, drainage_mean, &
            ...
            & coastalflow_mean, hydrographs, flood_frac, flood_res, ok_no_drainage)
       !
    ...
    IF ( .NOT. almaoutput ) THEN
       ...
       IF (dofloodplains .OR. doponds) THEN
          CALL histwrite(hist_id, 'reinfiltration', kjit, reinfiltration, nbpt, index)
       ENDIF
    ...
    ELSE
       ...
       IF ( doirrigation ) THEN
          CALL histwrite(hist_id, 'Qirrig', kjit, irrigation, nbpt, index)
          CALL histwrite(hist_id, 'Qirrig_req', kjit, irrig_netereq, nbpt, index)
       ENDIF
       !
    ENDIF
    IF ( hist2_id > 0 ) THEN
       IF ( .NOT. almaoutput ) THEN
          ...
          IF (dofloodplains .OR. doponds) THEN
             CALL histwrite(hist2_id, 'reinfiltration', kjit, reinfiltration, nbpt, index)
          ENDIF
...
!    flood_res = zero
    ...
    CALL ioconf_setatt('LONG_NAME','Accumulated potential transpiration for routing/irrigation')
...
!_ ================================================================================================================================
  ...
  SUBROUTINE routing_flow(nbpt, dt_routing, floodout, runoff, drainage, &
       ...
       &                  coastalflow, hydrographs, flood_frac, flood_res, ok_no_drainage)
    !
    IMPLICIT NONE
    !
!! INPUT VARIABLES
...
!! LOCAL VARIABLES
    ...
    !
    !Chloe : 
      LOGICAL, INTENT(in)                        :: ok_no_drainage      !!logical to do not use error_reservoir (slow reservoir) when we impose no drainage
    transport(:,:) = zero
    transport_glo(:,:) = zero
    irrig_netereq(:) = zero
    irrig_needs(:,:) = zero
       ...
       DO ig=1,nbpt
          IF ( route_tobasin(ig,ib) .GT. 0 ) THEN
             ...
             flow = MIN(stream_reservoir(ig,ib)/((topo_resid(ig,ib)/1000.)*stream_tcst* & 
                  ...
                  & stream_reservoir(ig,ib)-min_reservoir)
...
!Chloe : 
           IF ( slow_reservoir(ig,ib) .LT. zero .AND. .NOT. ok_no_drainage ) THEN
         !    error_reservoir=.TRUE.
        !     EXIT loopresnbpt
          ENDIF
          !Chloe++
          IF  ( slow_reservoir(ig,ib) .LT. zero .AND.  ok_no_drainage )THEN
            slow_reservoir(ig,ib)=zero
          ENDIF
          !Chloe--
       ENDDO
    ENDDO loopresnbpt
    ...
    IF (error_reservoir) THEN
       ...
       !CALL ipslerr(3, 'routing_flow', 'ERROR, slow_reservoir can be safely be reset to zero.','','')
    ENDIF
    ...
    IF ( doirrigation ) THEN
       DO ig=1,nbpt
          ...
          IF ((vegtot(ig) .GT. min_sechiba) .AND. (humrel(ig) .LT. un-min_sechiba) .AND. &
             ...
             irrig_netereq(ig) = (irrigated(ig) / totarea(ig) ) * MAX(zero, crop_coef * transpot_mean(ig) - &
