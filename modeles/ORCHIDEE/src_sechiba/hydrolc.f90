...
!! $Date: 2012-11-08 15:24:07 +0100 (Thu, 08 Nov 2012) $
!! $Revision: 1054 $
!! \n
!_ ================================================================================================================================
...
MODULE hydrolc
  ...
  USE parallel          ! for all parallel modules
  USE slowproc 
!  USE Write_Field_p
...
!_ ================================================================================================================================
  ...
  !! 3. Water budget calculations:
    ...
    write(*,*) 'hydrolc 433: bqsb=',bqsb(1,1)
   ...
   write(*,*) 'hydrolc 436: shumdiag=',shumdiag(1,1)  
   write(*,*) 'hydrolc 438: bqsb=',bqsb(1,1)
  ...
  !! 4. Output
    ...
    IF ( .NOT. almaoutput ) THEN
       ...
       CALL histwrite(hist_id, 'runoff', kjit, run_off_tot, kjpindex, index) ! this is surface runoff = 5% of total runoff
       CALL histwrite(hist_id, 'drainage', kjit, drainage, kjpindex, index) ! this 95% of total runoff
       ...
       CALL histwrite(hist_id, 'humrel',   kjit, humrel,   kjpindex*nvm, indexveg) ! this the transpiration stress factor
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
                IF (jv .EQ. 1) THEN
                   histvar(ji)=histvar(ji)+tot_bare_soil(ji)/vegtot(ji)*MAX((0.1-dss(ji,jv))*mx_eau_eau, zero)
                ELSE
...
!_ ================================================================================================================================  
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std), ALLOCATABLE, DIMENSION (:,:)            :: zdsp_g              !! Ancillary variables for initializing dsp and 
                                                                                !! dss (m)   
...
!!$        ENDDO
        ...
        !Config Units = [kg/m^2]
        CALL setvar_p (bqsb, val_exp, 'HYDROL_BQSB', mx_eau_eau*dpu_max)
        ...
        ! set inital value for hdry , see hydrolc_soil, step 8.4
        IF ( ok_var("hdry") ) THEN
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
  ...
  !! 3. Initial values of the mean soil layer depths and water contents
    ...
    DO ji = 1, kjpindex
      ...
      ENDIF
    ENDDO
      write(*,*) 'hydrolc 1376: mean_bqsb=',mean_bqsb(1)
      write(*,*) 'vegtot=',vegtot(1)
      write(*,*) 'bqsb=',bqsb(1,:)
      write(*,*) 'resdist=',resdist(1,:)
...
!! maxmass_glacier=3000 \f$kg m^{-2}\f$ (~ 10m snow) : critical snowmass for glaciers\n
...
!! fusion.\n
!! A special case occurs in areas with snowmass exceeding maxmass_glacier, where
!! all the snow in excess of maxmass_glacier adds to the total melting tot_melt.
...
!!$       ENDDO
   ...
   !! 3. On Land ice
    ...
    DO ji=1,kjpindex
      ...
      !! 3.4 Ice melt only if there is more than a given mass : maxmass_glacier, 
      !      i.e. only weight melts glaciers !
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
  ...
  !! 2. Updating soil moisture
    ...
    write(*,*) 'hydrolc 2537: gqsb=',gqsb(1,1)
    write(*,*) 'zeflux=',zeflux(1,1)
    write(*,*) 'transpir=',transpir(1,:)
    write(*,*) 'vevapnu=',vevapnu(1)
    write(*,*) 'zpreci=',zpreci(1,1)
    write(*,*) 'tot_melt=',tot_melt(1)
    write(*,*) 'reinfiltration=',reinfiltration(1)
    write(*,*) 'tot_bare_soil=',tot_bare_soil(1)
    write(*,*) 'vegtot=',vegtot(1)
    DO jv=1,nvm
      DO ji=1,kjpindex
      ...
      END DO
    ENDDO
    write(*,*) 'hydrolc 2573: bqsb=',bqsb(1,1)
    write(*,*) 'gqsb=',gqsb(1,1)
  ...
  !! 3. We compute runoff and adjust the soil layers' depth and water content
    ...
    DO jv=1,nvm
      ...
      ENDDO
    ENDDO
    write(*,*) 'hydrolc 2666: bqsb=',bqsb(1,1)
  ...
  !! 5. Top layers that are very large or very dry can be deadlock situations for the Choisnel scheme
    ...
    write(*,*) 'hydrolc 2701: bqsb=',bqsb(1,1)
    write(*,*) 'gqsb,gdrainage=',gqsb(1,1),gdrainage(1,1)
    DO jv=1,nvm
      DO ji=1,kjpindex
        
        !! 5.1 If the top layer is almost dry, we merge the two layers 
        IF ( ABS(dsp(ji,jv)-dsg(ji,jv)) .LT. min_resdis ) THEN ! min_resdis = 2.e-5 (constantes_soil.f90)
            bqsb(ji,jv) = bqsb(ji,jv) + gqsb(ji,jv)
        ...
        gqseuil = min_resdis * deux * ruu_ch(ji) ! min_resdis = 2.e-5 (constantes_soil.f90)
        eausup = dsg(ji,jv) * ruu_ch(ji)
        wd1 = .75*eausup
        
        IF (eausup .GT. gqseuil) THEN ! dsg > 4.e-5 m
  ...
  !! 6. We want the vegetation to share a common bottom moisture:
    ...
    !!  6.1 Average of bottom and top moisture over the "veget" fractions
      write(*,*) 'hydrolc 2744: tot_bare_soil=',tot_bare_soil(1)
      write(*,*) 'vegtot=',vegtot(1)
      write(*,*) 'bqsb=',bqsb(1,:)
      write(*,*) 'gqsb=',gqsb(1,:)
    mean_bqsb(:) = zero
...
!        ENDDO
      ...
      write(*,*) 'hydrolc 2849: tot_bare_soil=',tot_bare_soil(1)
      write(*,*) 'vegtot=',vegtot(1)
      write(*,*) 'bqsb=',bqsb(1,:)
      write(*,*) 'gqsb=',gqsb(1,:)
      mean_bqsb(:) = zero
...
!MM veget(:,1) BUG ??!!!!!!!!!!!
  ...
  !! 8. Diagnostics which are needed to carry information to other modules:
    ...
    write(*,*) 'hydrolc 2987: mean_bqsb=',mean_bqsb(1)
    write(*,*) 'mx_eau_var=',mx_eau_var(1)
    DO jd = 1,nbdl
      DO ji = 1, kjpindex
!         write(*,*) 'hydrolc tmp 2994: ji,jd=',ji,jd
         IF ( tmp_dl(jd+1) .LT. mean_dsg(ji)) THEN ! mean_dsg = mean depth of water in top layer in meters
            ...
            ! relative moisture
            shumdiag(ji,jd) = mean_gqsb(ji)/mx_eau_var(ji)
!            write(*,*) 'hydrolc tmp 2995: shumdiag(ji,jd)=',shumdiag(ji,jd)
         ELSE 
            IF ( tmp_dl(jd) .LT. mean_dsg(ji)) THEN
               ...
               shumdiag(ji,jd) = gtr*mean_gqsb(ji)/mx_eau_var(ji) + &
                    & btr*mean_bqsb(ji)/mx_eau_var(ji)
!               write(*,*) 'hydrolc tmp 3005: shumdiag(ji,jd)=',shumdiag(ji,jd)
            ELSE

               ! If the diagnostic soil level entirely fits into the mean bottom soil layer depth,
               ! they have the same relative moisture 
               shumdiag(ji,jd) = mean_bqsb(ji)/mx_eau_var(ji)
!              write(*,*) 'hydrolc tmp 3011: shumdiag(ji,jd)=',shumdiag(ji,jd)
            ENDIF
         ENDIF
         shumdiag(ji,jd) = MAX(MIN(shumdiag(ji,jd), un), zero)
!         write(*,*) 'hydrolc tmp 3015: shumdiag(ji,jd)=',shumdiag(ji,jd)
      ENDDO
    ENDDO
