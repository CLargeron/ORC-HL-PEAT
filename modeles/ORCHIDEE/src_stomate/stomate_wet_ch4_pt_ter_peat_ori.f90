...
!Chloe : 
    ...
    REAL(r_std),DIMENSION (npts,nvm), INTENT(in)       :: npp      !! Net primary productivity  
...
!    pwater=-3
   ...
   ! Chloe 15/12/14 : 
   ! Substrat= fin*f(T) cf Walter : ici on fait dépendre  
    substrat(:) = 0.0
    DO i=1,11
      DO ipoint = 1,npts
        !Chloe change le Carbone Labile  en NPP : 
        substrat(ipoint) = substrat(ipoint) + veget_max(ipoint,i)*carbon(ipoint,1,i) 
    !   substrat(ipoint) = substrat(ipoint) +  veget_max(ipoint,i)*npp(ipoint,i)
    !Chloe --
      ENDDO
    ENDDO
...
!!$!              source=forg(i)*(rq10**rexp)*carbon(itime)*(0.001239567)
!!$!            source=5.0*(rq10**rexp)
!!$                    source=forg(ipoint,i)*(rq10**rexp)*substrat(ipoint)*alpha
!!$                ELSE
!!$                    source=0.
!!$                ENDIF
!!$                q=source
