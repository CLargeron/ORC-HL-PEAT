...
CONTAINS
  ...
  SUBROUTINE ch4_wet_flux_density_peat ( npts,dt,stempdiag,tsurf,tsurf_year,veget_max,veget,&
     ...
     !Chloe
     nstm, wt_soil,npp_longterm, npp_week,maxnppweek_thisyear,wtold)  
...
!Chloe : WARNING DEBUG TEST change le in en inout
    REAL(r_std),DIMENSION (npts,nstm), INTENT(in)      :: wt_soil  !!Water Table position at each time step (mm)      
    ...
    REAL(r_std),DIMENSION (npts,nvm), INTENT(in)       :: maxnppweek_thisyear !! Max NPP annuel (à partir de la npp_week  
    ! 0.2 modified fields
...
!new dimension
    ...
    INTEGER(i_std),DIMENSION(npts)               :: n2 !Chloe :add dépendence en npts    
    INTEGER(i_std)                               :: n3 
!new dimension
...
!Chloe--
    ...
    quotient(:) = 0
!Chloe++  
  !CHLOE on ne prend que les propriétés tveg et rdepth (profondeur racinaire) du
  !peat :
  !  DO i=1,11
  !    DO ipoint = 1,npts
...
!Chloe--    
  ...
  !    ENDIF
  !  ENDDO
!Chloe a commenté et remplace pour le cas du peat : 
!Chloe++
...
!Chloe--
   ...
   tveg(:)= tveg_v(14)  
   !Chloe--
    tveg(:) = 0.5*tveg(:)/15.  !chloe : ca veut dire quoi ça ???
...
!    pwater=-3
    ...
    !!Chloe : ICI la water table est fixé par pwater_peat (qui vaut -3 cm)
    rwater(:) = pwater_peat
...
!Chloe++ :
!Méthode de Bruno (dépend du stock de carbone) : 
   DO ipoint = 1, npts
...
!Chloe add r0pl comme modèle Walter :
...
! using annual mean temperature and total annual NPP inppt
   ...
   ENDDO
!Chloe add fin comme modèle Walter en l'adaptant :
...
!-> cf. stomate_io
!!CHLOE CECI EST INITALISE DANS STOMATE_IO
...
!    DO i=150,ns
!Chloe : On étand de ns à nvert
!idéalement prendre Tair après ns
     DO i=150,ns 
      DO ipoint = 1,npts
...
!CHLOE : ON MODIFIE LA WATER TABLe : 
   ...
   ! ENDDO
!Chloe : nvert-4 évoque la wt au max sauf que nvert est au dessus de ns (soil
!surface), donc c'était une erreur de raisonnement, juste ?
!Chloe++ :
!Cas du peat : 
!no = 1 partout
!nw : position nappe
...
!Chloe TEST DEBUG : A EFFACER : 
    ...
    DO ipoint = 1,npts
        n0(ipoint)=ns-MIN(isoil(ipoint),nthaw)
        nw(ipoint)=MAX(n0(ipoint),ns-NINT(wt_soil(ipoint,4)/10))
       ! garde fou : (normalement n'arrive jamais car wt max fixé à 10cm)
       ! au max la WT est à 20cm au dessus du sol :
       !04/01/15 WARNING : nvert -4 vient de l'eq 10 dans Walter 2000
       ! Est ce que l'on doit imposer le même schema dans le cas du peat ? 
       !TEST DEBUG :
...
!!!!!CHLOE 11/02/2015 : ON REINITIALISE uo (qui est init dans stomate_io)
        DO i=1,nvert
          DO ipoint = 1,npts
             IF (i .LE. ns) THEN 
...
!CHLOE décommente ce cas qui est désormais possible : 
...
!Chloe 06/01/15 : 
!On impose n3 fixe à 15 cm au dessus de la surface :
!n3=ns+15
!Chloe on test comme Walter :
!n3=n2+4
!voir conditions ci dessus dépend de n2(ipoint) 
...
!Chloe++ :
 ...
 !on remet wtdiff à zero pour le moment : 
!Chloe apparemment c'est pas comme ça que ça marche...
! wtdiff(:)=0.0
!    DO ipoint=1,npts
!        wtdiff(ipoint)=ABS(NINT(wt_soil(ipoint,4)/10)- NINT(wtold(ipoint)/10))
!        wtold(ipoint)=wt_soil(ipoint,4)
!    ENDDO
!Walter faisait comme ça : 
!* if water table falls (i.e. nw (new water tabel) < nwtm1 (old water table)):
...
! Chloe réadapte ce qu'avait fait Walter : 
! On ne prend en compte la wtdiff que si wtnew est inférieur a wtold
...
! Chloe : ijump dépend de ipoint : donc fait une grande boucle sur ipoint
! (soupir))
!Chloe 06/01/15 : pour l'instant on va faire le calcul tout le temps
! et si jamais on est dans le cas ou ijump=0 alors on met tous à zero
...
!!Chloe 06/01/15:
!Chloe : On complète le 4ème cas de la même manière que les autres
!ceci est important pour la résolution de l'équation de la diffusion de fick par la méthode des différences
! finies de Crank Nicholson (tridiagonalisation nécessaire car equation non linéaire)
   ...
   !diffusion coefficients water-air interface (water --> air (since above ns))
        ...
        ELSEIF ((i .GE. n2(ipoint)+2) .AND. (i .LE. n3-1)) THEN
            ...
            !j1(ipoint)=n2(ipoint)+1 !=n2+1
            !j2(ipoint)=n2(ipoint)+2 !=n2+2
            !j3(ipoint)=n2(ipoint)+3 !=n2+3i 
            amhalf=diffair
            aphalf=diffair
            p=0.
            a(ipoint,i,j1(ipoint))=-rkh*amhalf
            a(ipoint,i,j2(ipoint))=2.+rkh*(amhalf+aphalf-h**2*p)
            a(ipoint,i,j3(ipoint))=-rkh*aphalf
            !! Gerhard dans ce cas i ne peut pas etre egale a 153
        ENDIF
     ENDIF !Cas 4
...
! transform from concentration into partial pressure unit
                ...
                aold4=2.*rk*q
                !aold4=2.*rk*q
                !aold1=MIN(rkh*amhalf,0.99)
                !aold3=MIN(rkh*aphalf,0.99)
                !aold2=2.0-aold1-aold3+rkh*h**2*p
                b(ipoint,i)=aold1*uo(ipoint,i-1)+aold2*uo(ipoint,i)+aold3*uo(ipoint,i+1)+aold4
...
! transform from concentration into partial pressure unit
        ...
        ENDIF !end cas 1
     ENDDO !end do ipoint
  ENDDO ! end i = 1 à n3-1

!!CAS 4 à mettre en 2eme position :
!Chloe CAS 4 : nw > ns : 
      ...
      DO ipoint = 1,npts
         ...
         ! diffusion coefficients water-air interface (water --> air (since above ns))
            ...
            ! ELSEIF (i .EQ. n2(ipoint)+1) THEN
             ...
             ! ELSEIF ((i .GE. n2(ipoint)+2) .AND. (i .LE. n3-1)) THEN
                DO i=n2(ipoint)+2,n3-1
                !C'est a dirr au dessus de l'eau stagnant donc u=catm !
                amhalf=diffair
                aphalf=diffair
                p=0.
                q=0.
...
!**************************
        ...
        !DO i=n0,n3
        !DO i=1,n3
        !!CHLOE TEST la diff plant au max à n2 :
        DO ipoint =1,npts
        !! La diffusion de plantes est comprise entre la profondeur des racines
        !nroot et la surface du sol =151 : ici i min vaut 114 car root=0 avant.  
           DO i=1,ns
           !DO i=1,n2(ipoint)
            IF (i .GE. n0(ipoint)) THEN
...
!negconc : concentration négative (if uo>0)
