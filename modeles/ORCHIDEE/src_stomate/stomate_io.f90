...
!Chloe +: 
    REAL(r_std),DIMENSION(npts,nstm),INTENT(out) :: wtsoil_daily
!Chloe -
...
!Chloe :
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out) :: maxnppweek_thisyear

    !Chloe:
!    REAL(r_std),DIMENSION(npts),INTENT(out)      :: wtold
    ...
    !Chloe :
    REAL(r_std),DIMENSION(npts,nvm),INTENT(out) :: npp_week
    ! biomass (gC/m**2)
...
!Chloe
    ...
    REAL(r_std),DIMENSION(npts,nvert),INTENT(out) :: uo_peat
    REAL(r_std),DIMENSION(npts,nvert),INTENT(out)  :: uold2_peat
...
!Chloe ++
    ...
    wtsoil_daily(:,:) = val_exp
    var_name = 'wtsoil_daily'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, nstm  , 1, itime, &
         &              .TRUE., wtsoil_daily, 'gather', nbp_glo, index_g)
    IF (ALL(wtsoil_daily(:,:) == val_exp)) wtsoil_daily(:,:) = zero
...
!Chloe :
    maxnppweek_thisyear(:,:) = val_exp
    var_name = 'maxnppweek_thisyear'
    CALL restget_p (rest_id_stomate, var_name, nbp_glo, nvm  , 1, itime, &
         &              .TRUE., maxnppweek_thisyear, 'gather', nbp_glo, index_g)
    IF (ALL(maxnppweek_thisyear(:,:) == val_exp)) &
         &     maxnppweek_thisyear(:,:) = zero
!Chloe
    ...
    !var_name='wtold'
    ! CALL restget_p (rest_id_stomate, var_name, nbp_glo, 1     , 1, itime, &
    !          &              .TRUE., wtold, 'gather', nbp_glo, index_g)
    !   IF (ALL(wtold(:) == val_exp)) wtold(:) = zero
...
! Wetland CH4 Methane
   ...
   var_name = 'uo_peat'
   CALL restget_p (rest_id_stomate, var_name, nbp_glo,   nvert, 1, itime, &
       &              .TRUE.,uo_peat , 'gather', nbp_glo, index_g)
   IF (ALL(uo_peat(:,:) == val_exp)) THEN
      DO nivo=1,nvert
         IF (nivo .LE. ns) THEN
            uo_peat(:,nivo) = scmax
         ELSE
            uo_peat(:,nivo) = CH4atmo_CONC
         ENDIF
      ENDDO
   ENDIF
   ...
   var_name = 'uold2_peat'
   CALL restget_p (rest_id_stomate, var_name, nbp_glo,   nvert, 1, itime, &
        &              .TRUE.,uold2_peat , 'gather', nbp_glo, index_g)
   IF (ALL(uold2_peat(:,:) == val_exp)) THEN
      DO nivo=1,nvert
         IF (nivo .LE. ns) THEN
            uold2_peat(:,nivo) = scmax
         ELSE
            uold2_peat(:,nivo) = CH4atmo_CONC
         ENDIF
      ENDDO
   ENDIF
...
!Chloe
    ...
    ! daily net primary productivity (gC/m**2/day)
!Chloe++
    REAL(r_std),DIMENSION(npts,nstm),INTENT(in) :: wtsoil_daily
!CHloe--
    ...
    ! this year's maximum weekly NPP
    REAL(r_std),DIMENSION(npts,nvm),INTENT(in) :: maxnppweek_thisyear
!Chloe:
!    REAL(r_std),DIMENSION(npts),INTENT(in) :: wtold 
!Chloe--    
    ! last year's annual GDD0
...
!Chloe :
    REAL(r_std),DIMENSION(npts,nvm),INTENT(in) :: npp_week
    ! biomass (gC/m**2)
...
!Chloe
  REAL(r_std),DIMENSION(npts,nvert),INTENT(in) :: uo_0
    ...
    REAL(r_std),DIMENSION(npts,nvert),INTENT(in) :: uo_peat
    REAL(r_std),DIMENSION(npts,nvert),INTENT(in)  :: uold2_peat
...
var_name = 'uo_0'
    ...
    var_name = 'uo_peat'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvert, 1, itime, &
         &              uo_peat, 'scatter', nbp_glo, index_g)

    var_name = 'uold2_peat'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvert, 1, itime, &
         &              uold2_peat, 'scatter', nbp_glo, index_g)
...
!Chloe++
    var_name = 'wtsoil_daily'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nstm, 1, itime, &
         &                wtsoil_daily, 'scatter', nbp_glo, index_g)
    !-
!Chloe--
...
! Chloe :
    var_name = 'maxnppweek_thisyear'
    CALL restput_p (rest_id_stomate, var_name, nbp_glo, nvm, 1, itime, &
         &                maxnppweek_thisyear, 'scatter', nbp_glo, index_g)
