  !!!! CHLOE : ATTENTION CES MODULES TER0 ET WET1 2 3 4 NE SONT PAS REALISTE CAR
  !!!! LES VALEURS DES COEFFICIENTS DE DIFFUSIONS NE SONT PAS DEFINIES.
  !!!! A NE PAS UTILISER
...
!=>  je dois modifier la valeur max d indices des boucles  n2,n3=f(ipoint)
    ...
    DO ipoint = 1,npts
      ...
      ENDIF
    ENDDO
    !write(*,*) 'Chloe DEBUG substrat ter0', substrat(:)
...
! if water table is at soil surface
      IF (nw(ipoint) .EQ. ns) THEN
          ...
          rvmax(ipoint)=0.
          !write(*,*) 'Chloe TER0 diffsw value',diffsw    
      endif
    ENDDO
    ...
    DO i=1,n3   
      DO ipoint = 1,npts
        IF (i .GE. n0(ipoint)) THEN
           ...
           !write(*,*) 'Chloe DEBUG ter0  uo, catm ', uo(ipoint,i), catm
        ENDIF
      ENDDO
    ENDDO
...
! transform from concentration into partial pressure unit
        ...
        !   write(*,*) 'ter0 b,aold1,aold2,aold3,amhalf,aphalf',b(1,n0(1)),aold1,aold2,aold3,amhalf,aphalf  
!        ENDIF
        ENDDO
...
! calculate NEW concentration profiles uo(i)
        ...
        DO j=2,n3
          DO ipoint =1,npts
            IF (j .LE. ndim(ipoint)) THEN
              ...
              !write(*,*)'TER0 triu',triu(1,2:20),'trir',trir(1,2:20),'tria',tria(1,2:20),'bet',bet(1)
            ENDIF
          ENDDO
        ENDDO
        ...
        !    write(*,*) 'TER0 triu avant j=n3-1..', triu(1,:)
                triu(ipoint,j)=triu(ipoint,j)-gam(ipoint,j+1)*triu(ipoint,j+1)
        !    write(*,*) 'TER0 triu j=n3-1,1,-1', triu(1,:)
            ENDIF
          ENDDO
        ENDDO      
        ...
        DO i=1,n3
          DO ipoint =1,npts  
            IF (i .GE. n0(ipoint)) THEN
      !        write(*,*) 'TER0 triu', triu(1,i-n0(1)+1)
...
!         IF (iday .EQ. 1) THEN
!            write(*,*) 'cplant', cplant
!             WRITE(*,*) 'TER0 matrice uo apres plante',uo
!         ENDIF
!         ENDDO
