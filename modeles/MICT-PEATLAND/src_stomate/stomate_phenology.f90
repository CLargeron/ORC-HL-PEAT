...
!_ ================================================================================================================================
  ...
  SUBROUTINE phenology (npts, dt, PFTpresent, &
    ...
    !
    REAL(r_std), DIMENSION(npts,nvm,nparts), INTENT(inout)              :: biomass              !! biomass @tex ($gC m^{-2} of 
                                                                                                !! ground$) @endtex
...
!_ ================================================================================================================================
    ...
    DO j = 2,nvm
       ...
       WHERE ( ( time_lowgpp(:,j) .GE. lowgpp_time(j) ) .AND. &
    ...
    ELSEWHERE
       histvar=zero
    ENDWHERE
    CALL histwrite (hist_id_stomate, 'ALLOW_INITPHENO', itime, histvar, npts*nvm, horipft_index)
    ...
    ELSEWHERE
       histvar=zero
    ENDWHERE
    CALL histwrite (hist_id_stomate, 'BEGIN_LEAVES', itime, histvar, npts*nvm, horipft_index)
          ...
          IF ( begin_leaves(i,j) ) THEN
             ...
             IF ( biomass(i,j,ileaf) .LT. lm_min(i) ) THEN
                ...
                IF ( always_init .AND. ( biomass(i,j,icarbres) .LT. bm_wanted(i) ) ) THEN
                   !NV passage 2D
                   co2_to_bm(i,j) = co2_to_bm(i,j) + ( bm_wanted(i) - biomass(i,j,icarbres) ) / dt
