...
!_ ================================================================================================================================
 ...
 !! 0. Variable and parameter declaration
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)        :: ind                      !! Number of individuals 
                                                                                       !! @tex $(m^{-2})$ @endtex    
    REAL(r_std), DIMENSION(npts,nvm,nparts), INTENT(inout) :: biomass                  !! Biomass @tex $(gCm^{-2})$ @endtex    
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)        :: veget_lastlight          !! Vegetation cover after last light 
                                                                                       !! competition (unitless;0-1)     
    REAL(r_std), DIMENSION(npts,nvm,nparts), INTENT(inout) :: bm_to_litter             !! Biomass transfer to litter per timestep
                                                                                       !! @tex $(gCm^{-2})$ @endtex    
    ...
    ! INTEGER(i_std)                                         :: num_grass              !! [DISPENSABLE] Number of grass PFTs present in the 
                                                                                       !! grid box (unitless)       
             ...
             DO j = 2,nvm ! Loop over #PFTs
                ...
                IF ( PFTpresent(i,j) .AND. natural(j) ) THEN
                   ...
                   ELSE
                      ...
                      ! survive(j) = ( 0.01 / REAL( num_grass,r_std ) ) / ind(i,j)  !( 0.01 = grass_mercy previously)
