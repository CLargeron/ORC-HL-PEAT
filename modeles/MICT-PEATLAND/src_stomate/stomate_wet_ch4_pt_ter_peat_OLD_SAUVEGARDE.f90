...
!Chloe TEST DEBUG : A EFFACER : 
    ...
    DO ipoint = 1,npts
        n0(ipoint)=ns-MIN(isoil(ipoint),nthaw)
        IF (n0(ipoint) .NE. 1.) THEN 
        write(*,*) 'BUUUUUUUUUUUUG n0',n0(ipoint)
        ENDIF
         ...
         ! diffusion coefficients water-air interface (water --> air (since above ns))
           ...
           ! ELSEIF ((i .GE. n1(ipoint)+1) .AND. (i .LE. n2(ipoint)-1)) THEN
            ...
            !ELSEIF ((i .GE. n2(ipoint)+2) .AND. (i .LE. n3-1)) THEN
                ...
                aold4=2.*rk*q
                b(ipoint,i)=aold1*uo(ipoint,i-1)+aold2*uo(ipoint,i)+aold3*uo(ipoint,i+1)+aold4
                 IF (b(ipoint,i) .LT. 0.) THEN
                !Il est ici le probleme
                write(*,*) 'NEG b CAS4 i,b,nw,uo(i-1),uo(i),uo(i+1)',i,b(ipoint,i),nw(ipoint),uo(ipoint,i-1),uo(ipoint,i),uo(ipoint,i+1)
                ENDIF
                ENDDO
           ! ENDIF
         ENDIF !end of cas 4 
...
DO ipoint=1,npts
IF (fluxdiff(ipoint) .GT. 100) THEN
write(*,*) 'Chloe diff 100, fluxbub',fluxbub(:),'negconc',negconc(:),'fluxbubdif',fluxbubdiff(:),'wtdiff',wtdiff(:),'wt_soil', wt_soil(ipoint,4), 'wtold',wtold(:) 
ENDIF
ENDDO
