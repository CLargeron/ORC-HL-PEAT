...
!_ =================================================================================================================================
  ...
  SUBROUTINE season (npts, dt, EndOfYear, veget, veget_max, &
       ...
       time_hum_min, hum_min_dormance, herbivores, tsurf_daily, tsurf_year)
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)           :: npp_daily         !! daily net primary productivity @tex ($gC m^{-2} 
                                                                                !! day^{-1}$) @endtex
    REAL(r_std), DIMENSION(npts,nvm,nparts), INTENT(in)    :: biomass           !! biomass @tex ($gC m^{-2} of ground$) @endtex
    REAL(r_std), DIMENSION(npts,nvm,nparts), INTENT(in)    :: turnover_daily    !! Turnover rates @tex ($gC m^{-2} day^{-1}$) 
                                                                                !! @endtex 
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(in)           :: when_growthinit   !! how many days ago was the beginning of the 
                                                                                !! growing season (days) 
!Chloe : 
REAL(r_std), DIMENSION(npts), INTENT(in)           :: tsurf_daily ! daily surface temperature
    !
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)        :: maxgppweek_thisyear       !! this year's maximum weekly GPP 
                                                                                        !! @tex ($gC m^{-2} week^{-1}$) @endtex 
!Chloe :
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)        :: maxnppweek_thisyear       !! this year's maximum weekly NPP 
                                                                                        !! @tex ($gC m^{-2} week^{-1}$) @endtex 
    ...
    REAL(r_std), DIMENSION(npts,nvm,nparts), INTENT(inout) :: turnover_longterm         !! "long term" turnover rate 
                                                                                        !! @tex ($gC m^{-2} year^{-1}$) @endtex
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)        :: gpp_week                  !! "weekly" GPP @tex ($gC m^{-2} day^{-1}$) 
                                                                                        !! @endtex 
!Chloe
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)        :: npp_week                  !! "weekly" NPP @tex ($gC m^{-2} day^{-1}$) 
                                                                                        !! @endtex 
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)        :: time_lowgpp               !! duration of dormance (days)
...
!!$    ENDIF
    ...
    ENDWHERE
!Chloe :
    WHERE ( npp_week .GT. maxnppweek_thisyear )
       maxnppweek_thisyear = npp_week
    ENDWHERE
