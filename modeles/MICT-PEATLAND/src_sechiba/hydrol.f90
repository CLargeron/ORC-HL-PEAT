...
!Isa : conductivité hydraulique moyenne sur la maille, tenant compte des soiltiles ie des préf de la végétation
...
!Isa pour gel dans le sol 
 ...
 !LOGICAL, SAVE                                     :: ok_routage_peat !! défini dans sechiba 
...
! Isa pour écriture des k_lin, à faire une fois !
!caveats : ce n'est pas la façon la plus rigoureuse de faire...(cf le dernier kjpindex qui devrait être kjpij)
!Chloe use k_lin_write d Isa, garder kk_moy non commenté, Comment Debut
!DO ji=1,kjpindex
!	do jst = 1, nscm
!		do i=1, imax
...
!Isa
    if ( control%ok_freeze ) then
        ...
        if (ok_sat30cm) write(*,*) 'Chloé : ok_sat30cm'
        
        ok_stagnant= .FALSE.
        CALL getin_p('OK_STAGNANT', ok_stagnant)
        if (ok_stagnant) write(*,*) 'Chloé : ok_stagnant'
       ...
       CALL getin_p('OK_REINFILT_PEAT',  ok_reinfilt_peat)
        if ( ok_reinfilt_peat) write(*,*) 'Chloé : ok_reinfilt_peat'
    ...
    ! CR: verif compatibilité des options:
    if (ok_freeze_cwrr) then
        if (ok_gel_thd) then
...
ENDIF
    ...
    DO jst=1,nstm
       ...
       ENDDO
    ENDDO
!Isa : correction du calcul de shumdiag pour le mettre sur l'échelle diag. soilmoist n'est pas concerné.
...
!ISA 1
    !
    ! 1.1. calcul de la température sur l'échelle hydro (nslm) sur la base du stempdiag (échelle diagnostique nbdl) 
    !
...
!Isa :contenu en eau de la couche 1. on transfère si nécessaire une partie du rootsink de la couche 1, excédentaire par rapport 
!au contenu en eau présent de cette couche,sur le rootsink de la couche 2.
       ...
       !
        !
       !Chloe++ Réinfiltration de l'eau présente dans le réservoir stagnant
       DO ji = 1, kjpindex
...
if (ok_freeze_cwrr) then
	do ji =1, kjpindex
		...
		mcl(ji,jsl,:)= MIN(mc(ji,jsl,:),mcr(njsc(ji))+(1-profil_froz_hydro_ns(ji,jsl, :))*(mc(ji,jsl,:)-mcr(njsc(ji))))
                !borne inférieure mc pour les cas under_mcr
        enddo
	enddo
...
!Isa : in the following, mc -> mcl
    ...
    !! 5.3 compute matrix coefficients
       ...
       DO ji = 1, kjpindex
          ...
          tmat(ji,jsl,3) = zero
          rhs(ji,jsl) = ep(ji,jsl)*mcl(ji,jsl-1,jst) + fp(ji,jsl)*mcl(ji,jsl,jst) &
!Isa : introduction du free_drain_coef, mis à 1, en attente de correction
 		& + (b(ji,jsl-1)+b(ji,jsl) & 
    ...
    !! 6.2 check if really atmosphere limiting
       DO ji = 1, kjpindex
          ...
          !- Isa : précision : avec les mc non modifiés par le hydrol_soil_tridiag précédent !
          !ISA 3: mc -> mcl
          !-
...
!Isa : correction pr ajout de free_drain_coef
       ...
       !Isa precision : si on est en under_mcr avant la résol précédente on ne
...
!       write(*,*) 'dz(2,1)=',dz(2,1)
    ...
    ! (returnflow introduit par le haut) !!??Aurelien: are you sure
          ...
          ! ae_ns+ru_ns=precisol_ns+irrigation-q0 
          ! Isa: qflux00 = tt ce qui est rentré dans le sol entre la fin et le
          ! début de la routine. Donc:
...
!! Mais après avoir calculé le ruissellement quand meme, évident non ?
    ! Chloe++ Stockage du réservoir d'eau stagnante                                                                     
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!   DO ji = 1, kjpindex
...
!       write(*,*) 'hydrol 3910'
    ...
    !! 9 Then compute the temporary surface water and correct the outgoing runoff
       !IF ( .NOT. doponds ) THEN
        !Chloe : 
        ! if stagnant : wat2inf=0 ici, water2infilt prend les quantités de runoff
        ! total auparavant sans la prise en compte de la pente.
        ! Cette méthode permet de ne pas avoir un tmc > tmcs = 860kg/m2 si
        ! mcs=0.43
...
!Chloe : mettre le ok_stagnant après les calculs et corrections appliqués au
!runoff : 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   !DO ji = 1, kjpindex
!  !     !We already are in loop soiltile
                   ...
                   wt_soil(ji,jst)=-1.0_r_std
                    DO jsl=1,nslm
                        IF (mc(ji,jsl,jst) .GE. 0.39 .AND. ABS(wt_soil(ji,jst)+1.0_r_std) .LT. EPSILON(1.0_r_std) ) THEN
                            !wt_soil = depth du haut de la couche qui est saturé en eau
...
!Isa
 SUBROUTINE hydrol_soil_coef1(kjpindex,ins,njsc) !pr infiltration comme en sol non gelé
    !
    IMPLICIT NONE
    !
    INTEGER(i_std), INTENT(in)                        :: kjpindex         !! Domain size
...
!_ hydrol_soil_coef
  ...
  SUBROUTINE hydrol_soil_coef(kjpindex,ins,njsc)
    ...
    REAL(r_std)                                       :: mc_ratio
!Isa
    REAL(r_std)              :: mc_used,x,m, mc_incremente ! contenu en eau liquide réel 
    ...
    !
!Chloe+ nvan(ins) changé en nvan(njsc)   
    !
    DO jsl=1,nslm
       DO ji=1,kjpindex 
...
!(1-x)==profil_froz_hydro
    ...
    ! utilisé pour le calcul des pptés hydro uniquement => pas de pb si
        ! sous-saturation ni sursaturation (x limité par 1).
!
...
!!!!!Schizophrénie : 10/10/14 pourquoi tu met ca la chloé ?
