...
! calculate the earth's orbital angle (around the sun) in radians
  orbit = 2.0*pi*REAL(jday)/365.2425
!-
...
!---
    coszen(i) = MAX(zero, (sin(xlat)*sin(xdecl) &
...
!---
    IF (dump_weather) THEN
      ...
      CALL gather2D(tair, tair_g)
      CALL gather2D(pb, pb_g)
      CALL gather2D(qair, qair_g)
      CALL gather2D(swdown, swdown_g)
      CALL gather2D(rainf, rainf_g)
      CALL gather2D(snowf, snowf_g)
      CALL gather2D(u, u_g)
      CALL gather2D(v, v_g)
      CALL gather2D(lwdown, lwdown_g)
      IF (is_root_prc) THEN
         CALL weathgen_dump &
...
!-
  ...
  IF (is_root_prc) THEN
    ...
    ENDIF
  ENDIF
  CALL scatter2D(xchamp_g,xchamp)
...
!!$  CALL scatter2D(champout_g,champout)
  ...
  champout(:,:)=champout_g(:,:)
#else
  CALL scatter2D_rgen(champout_g,champout,n3)
#endif
