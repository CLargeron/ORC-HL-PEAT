...
!-
  ...
  LOGICAL :: debug
  LOGICAL, ALLOCATABLE, DIMENSION(:)         :: peatland ! Chloe
  REAL, ALLOCATABLE, DIMENSION(:,:)          :: wt_soil   ! Chloe 
  REAL, ALLOCATABLE, DIMENSION(:,:)          :: wt_soil2   ! Chloe 
  !attention alloue uniquement dans sechiba
...
!-
  ...
  totsize_1step = SIZE(soiltile(:,4))*KIND(soiltile(:,4)) + &
  !Chloe
       SIZE(wt_soil(:,4))*KIND(wt_soil(:,4)) + & 
       SIZE(wt_soil2(:,4))*KIND(wt_soil2(:,4)) + &
...
!-
 ...
 &   rest_id_sec, hist_id_sec, hist_id_sec2, rest_id_sto, hist_id_stom, &
 &   hist_id_stom_IPCC, co2_flux, fco2_lu, peatland, wt_soil,wt_soil2)
  ! correct date
